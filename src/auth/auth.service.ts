...
export class AuthService {
  ...
  async forgot(forgotDto: ForgotDto) {
    ...
    if (existingUser) {
      ...
      await this.memcachedService.set(userEmail, randomNumber, 360);
  ...
  async validateOtp(creds) {
    const { email, otp } = creds;
    const otpCached = await this.memcachedService.get(email);
  ...
  private decryptData(ciphertext: string, key: string, iv: string) {
  ...
  private encryptData(data, key) {
    ...
    hmac.update(ciphertext + ivString);
  ...
  async login(data) {
    ...
    const decryptedData = this.decryptData(ciphertext, key, iv);

    console.log('Decrypted data from Front-end', decryptedData);
